...
// внешнее возвращаемое значение это внутренняя функция
...
// 1. Это функция, которая должна быть выполнена
// после того, как другая функция завершит работу
// 2. Функция обратного вызова — это подход, который гарантирует, 
// что некий фрагмент кода будет вызван только после того, как
// какой-то другой код завершит выполнение.
...
console.log(learnJS('JavaScript', process));
...
// Это такая функция которая может принимать другие функции в качестве аргументов
// и возвращать их в качестве результатов
...
// ===== Чистая функция ===============================================
// Это функция которая выдает один и тот же результат
...
// =====  IIFE (выражения функций с немедленным вызовом)
// Немедленно вызываемые функции используются, когда вы хотите,
// чтобы ваш код выполнялся сразу после его определения.
